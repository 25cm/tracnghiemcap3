...
class UpDownController extends Controller {
    ...
    private function uploadFile($path, $control_number, $publish_type) {
        ...
        Log::infoLog('method='.__FUNCTION__.';user_id='.$user->user_id.';Control number='.$control_number.';Path='.$path.';Start action;',$this->getRequest()->getParams());
        ...
        Log::infoLog('Upload file method='.__FUNCTION__.';user_id='.$user->user_id.';Control number='.$control_number.';File name='.$newFileName.';Full Path='.$path.';Publish type='.$publish_type.';End action;',$this->getRequest()->getParams());
        try {
        ...
        Log::infoLog('method='.__FUNCTION__.';user_id='.$user->user_id.';File name='.$newFileName.';Control number='.$control_number.';End action;',$this->getRequest()->getParams());
    }
     ...
     * @return boolean
     */
    public function setRowData($control_number, $publish_type, $status, $do_option, $do_base_paper, $do_customer_designation, $do_customer_format, $file_upload) {
    	$user = Auth_Info::getUser();
        ...
        try {
            ...
            if (strcmp($status, $this->_input->status) !== 0) {
                $row->status = $this->_input->status;
            } elseif ($this->canChangeStatus($do_option, $do_customer_designation, $do_customer_format, $do_base_paper, $file_upload)) {
            ...
            } elseif (strcmp($this->_input->ck_do_customer_designation, NULL) !== 0) {
            ...
            } elseif (strcmp($this->_input->ck_do_customer_format, NULL) !== 0) {
    ...
    public function requestAction() {
        ...
        Log::infoLog('method='.__FUNCTION__.';user_id='.$user->user_id.';Control number='.$control_number.';End action;',$this->getRequest()->getParams());
    }
    ...
    public function confirmRequestAction() {
        ...
        if (isset($_FILES['file_upload']['name']) && !empty($_FILES['file_upload']['name'])) {
            if (!is_dir(APPLICATION_PATH.self::URL_ROOT_UPLOAD.$yyyymm)) {
                mkdir(APPLICATION_PATH.self::URL_ROOT_UPLOAD.$yyyymm);
            }
            if (!is_dir(APPLICATION_PATH.self::URL_ROOT_UPLOAD.$yyyymm.self::SEPARATOR.$control_number)) {
                mkdir(APPLICATION_PATH.self::URL_ROOT_UPLOAD.$yyyymm.self::SEPARATOR.$control_number);
            }
        ...
        if ($upload_status_flag) {
            //Set data to TPublishInfo
            if ($this->setRowData($control_number, $publish_type, $status, $do_option, $do_base_paper, $do_customer_designation, $do_customer_format, $file_upload)) {
                //Set data to TPublishHist
                $db = Zynas_Db_Table::getDefaultAdapter();
                $db->beginTransaction();
                PublishHistCreator::createFromKey($control_number, $publish_type)->save();
                Log::infoLog('Save data to TPublishInfo method='.__FUNCTION__.';user_id='.$user->user_id.';Control number='.$control_number.';Publish type='.$publish_type.';End action;',$this->getRequest()->getParams());
                $db->commit();
                ...
                try {
                    ...
                    if (($upload_status_flag && strcmp($file_upload, null) !== 0 && $can_send_mail && strcmp($detail->status, TPublishInfos::STATUS_NOT_ISSUE) === 0)
                        ...
                        Log::infoLog('Send mail method='.__FUNCTION__.';user_id='.$user->user_id.';Control number='.$control_number.';Sender='.$user->user_id.';Receiver='.$to.';Mail body='.$body.';End action;',$this->getRequest()->getParams());
                    }
                } catch (Exception $e) {
                	Log::errorLog('Error log method='.__FUNCTION__.';Error Message='.$e->getMessage(), $this->getRequest()->getParams());
                    throw new Exception('Oops, An error occurred while sending email!', 0, $e);

                }

                if ($upload_status_flag) {
                    FlashMessenger::addSuccess(str_replace('${control_number}',$control_number,I003V));
                }

                $this->_redirect('/admin-unissue-hist/list');
            }
        }
        Log::infoLog('method='.__FUNCTION__.';user_id='.$user->user_id.';End action;',$this->getRequest()->getParams());
    }
