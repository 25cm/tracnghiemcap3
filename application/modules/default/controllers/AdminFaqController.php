...
class AdminFaqController extends Controller {
    ...
    public function confirmAddAction() {
        ...
        if(!Csrf::checkToken($this->_input->token)) {
            ...
            Log::errorLog('method='.__FUNCTION__.';user_id='.$user->user_id.';Error Message='.E061V,$this->getRequest()->getParams());
        ...
        try {
            ...
            $row->save();
            Log::infoLog('Add FAQ method='.__FUNCTION__.';user_id='.$user->user_id.';End action'.$row,$this->getRequest()->getParams());
    ...
    public function confirmEditAction() {
        ...
        if(!Csrf::checkToken($this->_input->token)) {
            FlashMessenger::addError(E061V);
            Log::errorLog('Erroe log method='.__FUNCTION__.';user_id='.$user->user_id.';Error Message='.E061V,$this->getRequest()->getParams());
            $this->_redirect('/admin-faq/list');
        }
        //変更画面から入力内容を取得する
        $id = $this->_input->id;
        ...
        try {
            ...
            $faqRow->save();
            Log::infoLog('Edit FAQ action='.__FUNCTION__.';user_id='.$user->user_id.';End action'.$row,$this->getRequest()->getParams());
            $db->commit();
    ...
    public function deleteAction() {
        ...
        if (!$this->getRequest()->isPost()){
            ...
            Log::infoLog('method='.__FUNCTION__.';user_id='.$user->user_id.';Error Message='.E062V,$this->getRequest()->getParams());
            $this->_redirect('/admin-faq/list');
        }
        ...
        try {
            ...
            $row->save();
            Log::infoLog('Delete FAQ action='.__FUNCTION__.';user_id='.$user->user_id.';End action'.$row,$this->getRequest()->getParams());
            $db->commit();
