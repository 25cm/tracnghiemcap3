...
if (strcmp(Auth_Info::getUser()->user_type, MUsers::DISCO_USER) === 0) {
        ...
        if (isset($_REQUEST['select_main_product_category_' . $productIndex])) {
            ...
            if (strcmp($_REQUEST['select_main_product_category_' . $productIndex], MCategorys::CATEGORY_DEVICE) === 0) {
                ...
                if (isset($_REQUEST['text_serial_number_' . $productIndex])) {
                    $serialNumberCheck = array('text_serial_number_' . $productIndex => array(Zynas_Filter_Input::ALLOW_EMPTY => False, array('StringLength', array('max' => '6', 'encoding' => 'UTF-8')), ), );
                ...
                if (strcmp($_REQUEST['select_model_serial_' . $productIndex], '01') === 0) {
                    ...
                    if (isset($_REQUEST['text_product_number_view_' . $productIndex])) {
                        $productNumberCheck = array('text_product_number_' . $productIndex => array(Zynas_Filter_Input::ALLOW_EMPTY => False, array('StringLength', array('max' => '25', 'encoding' => 'UTF-8')), array('DbRecordExistsTTM', array('field' => 'product_number', 'table' => 'm_product')), ), );
                        $validator = array_merge($validator, $productNumberCheck);
                    }
